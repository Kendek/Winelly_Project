...
import { SetNewUserData } from '../MServices/AccountService'
...
const SignUp = ({ flipForm }: Props) => {
  ...
  const SubmitSignup = async () => {
    ...
    if (!emailRegex.test(signupEmail)) return;
    if (!passwordRegex.test(signupPassword)) return;
    ...
    const result = await SetNewUserData(newUser);
